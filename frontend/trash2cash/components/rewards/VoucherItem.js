...
export default function VoucherItem({ voucher }) {
  const redeemVoucher = async () => {
    const res = await fetch(
      `http://trash2cashpersonal.onrender.com/api/rewards/update-voucher-instance/${voucher.id}/`,
      {
        method: "PATCH",
        headers: {
  ...
  const useVoucher = async () => {
    const res = await fetch(
      `http://trash2cashpersonal.onrender.com/api/rewards/use-voucher-instance/${voucher.id}/`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
