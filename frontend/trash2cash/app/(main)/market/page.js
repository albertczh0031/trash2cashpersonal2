...
export default function MarketplacePage() {
  ...
  async function handleSellerClick() {
    setLoading(true);
    try {
      const res = await fetch("https://trash2cashpersonal.onrender.com/api/get-seller-status/", {
        method: "GET",
        headers: {
  ...
  async function requestSellerVerification() {
    try {
      const res = await fetch("https://trash2cashpersonal.onrender.com/api/request-seller-verification/", {
        method: "POST",
        headers: {
          "Authorization": "Bearer " + localStorage.getItem("access"),
          "Content-Type": "application/json",
